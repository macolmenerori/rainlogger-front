...
- **Root**: `app/root.tsx` - i18n initialization (side-effect import), font imports, `App` component (with `ThemeProvider` + `AlertProvider` + `UserProvider` + `ServiceWorkerRegistration` + `<Outlet />`), `HydrateFallback` (blank dark screen shown while JS loads in SPA mode), and re-exports of `Layout`, `ErrorBoundary`, and `HydrateFallback`
...
- **Components**: `app/components/` - shared UI components
  ...
  - `ViewTabs/` - Tabbed data display components for the WatchLogs page. The WatchLogs page uses MUI `Tabs` to switch between three views (Table, Calendar, Graph), each receiving `RainLog[]` as a `data` prop. CalendarTab and GraphTab also receive `month` and `year` props
    - `TableTab/` - MUI Table displaying rain logs sorted by date ascending. Columns: Date (YYYY-MM-DD), Amount (measurement), Actions (Edit/Delete `IconButton`s). Responsive: `maxWidth: 450` on `sm+`, full-width on mobile. Edit button opens `UpdateLogModal`. Delete button opens `ConfirmDeleteModal`. Props: `data: RainLog[]`, `onDataChange?: () => Promise<void>` (called after successful update/delete to refresh data)
  ...
  - `ConfirmDeleteModal/` - MUI Dialog for confirming rain log deletion. Displays confirmation message with formatted date (day + translated month name). Delete button uses `color="error"` (red). Calls `deleteRainLog(id)` from rainloggerService. Features: loading spinner on delete button during submission, buttons disabled during submission, prevents backdrop close during submission, success/error alerts via `useAlert()`, `onDataChange` callback to trigger parent data refresh. Props: `open: boolean`, `log: RainLog`, `onClose: () => void`, `onDataChange?: () => Promise<void>`
  ...
  - `ServiceWorkerRegistration/` - Render-nothing component that registers the PWA service worker via `virtual:pwa-register`. Uses dynamic import inside `useEffect` with `.catch()` fallback for test environments. Mounted in `root.tsx` inside the provider tree
- **UI Components**: `app/ui/` - core UI infrastructure
  ...
  - `Layout/` - HTML shell component with dynamic lang attribute. Includes PWA `<link>` tags: manifest, favicons (SVG, PNG, ICO), and apple-touch-icon
...
- **Context**: `app/context/` - React context providers
  - `AlertContext/AlertContext.tsx` - `AlertProvider` component + `useAlert()` hook for global toast notifications (MUI Snackbar + Alert). See [Error Management](#error-management) section
...
- **Hooks**: `app/hooks/` - Custom React hooks
  - `useApi.tsx` - Generic data-fetching hook wrapping apiClient calls. Manages `data`, `loading`, `error` states with `refetch()`, `skip`, and `onError` callback support. See [useApi Hook](#useapi-hook) section
- **Services**: `app/services/` - API service modules
  ...
  - `rainloggerService.ts` - RainLogger API service using apiClient: `getRainLogs()`, `getRainLogsByDay()`, `getRainLogsByMonth()`, `postRainLog()`, `updateRainLog()`, `deleteRainLog()`
...
- **Static files**: `public/` - served at root
  - `site.webmanifest` - PWA web app manifest (name, icons, theme/background colors, display mode)
  - `icons/` - PWA and favicon icons
    - `web-app-manifest-192x192.png`, `web-app-manifest-512x512.png` - PWA install icons (both `any` and `maskable` purpose)
    - `apple-touch-icon.png` - iOS home screen icon (180x180)
    - `favicon.svg`, `favicon-96x96.png`, `favicon.ico` - Browser favicons
    - `logo.svg` - App logo
...
- **Charts**: Recharts 3 (BarChart, ResponsiveContainer for the GraphTab view)
...
- **PWA**: vite-plugin-pwa 1 (Workbox `generateSW` mode, auto-update)
...
```json
{
  ...
  "components": { "backButton": { "label": "..." }, "languageSwitcher": { "label": "..." }, "navbar": { "title": "...", "logout": "..." }, "errorBoundary": { "message": "..." }, "confirmDeleteModal": { "message": "Do you want to delete the {{day}} - {{month}} log?", "cancelButton": "...", "deleteButton": "..." }, "updateLogModal": { "title": "...", "measurementLabel": "...", "realReadingLabel": "...", "cancelButton": "...", "updateButton": "...", "errors": { "measurementRequired": "...", "measurementMin": "...", "measurementDecimals": "..." } }, "alert": { "newLog": { "success": "...", "error": "..." }, "updateLog": { "success": "...", "error": "..." }, "deleteLog": { "success": "...", "error": "..." }, "watchLogs": { "error": "..." }, "generic": { "error": "...", "networkError": "..." } } }
}
```
...
- **`deleteRainLog(id)`** — DELETE a rain log by ID. Returns `Promise<void>` (API returns 204 No Content)
- GET functions use `env.baseUrlRainlogger` as base URL, 3 retries with 2s base delay
- Return type: `ApiResponse<{ rainlogs: RainLog[] }>` for GETs, `ApiResponse<{ rainlog: RainLog }>` for POST/PUT, `void` for DELETE
...
- **Delete rain log**: `DELETE {BASE_URL_RAINLOGGER}/v1/rainlogger/rainlog/delete/{id}` — returns 204 No Content
...
function useApi<T>(
  ...
  options?: boolean | UseApiOptions        // Skip (boolean) or options object (default: false)
): UseApiReturn<T>;

interface UseApiOptions {
  skip?: boolean;                          // Skip fetching (default: false)
  onError?: (error: ApiError) => void;     // Callback on error (e.g., show alert)
}
...
**With onError callback (e.g., alert integration):**
```typescript
...
- The third parameter accepts either `boolean` (skip) or `UseApiOptions` object — backward compatible
...
- `onError` is called after `setError`, so both the callback and `error` state are available
...
- **Global setup**: `app/test/setup.ts` - jest-dom matchers, i18n initialization, MSW server lifecycle, `ResizeObserver` class mock (for recharts and MUI Select), `window.matchMedia` mock for MUI
- **Custom render**: `app/test/utils/test-utils.tsx` - wraps components with `ThemeProvider` + `AlertProvider` + `UserProvider` + `MemoryRouter`
...
- **Handlers**: `app/test/mocks/handlers/rainlogHandlers.ts` - MSW v2 handlers for rainlogger endpoints (GET filters, POST rainlog, PUT rainlog, DELETE rainlog)
...
The project uses a **global alert system** built on MUI Snackbar + Alert for transient toast notifications. Any component can trigger an alert with a severity level. See `app/context/AlertContext/AlertContext.docs.md` for detailed documentation.

### Architecture
- **AlertContext**: `app/context/AlertContext/AlertContext.tsx` - context provider, `useAlert()` hook, and Snackbar+Alert UI
- **Provider placement**: Inside `ThemeProvider`, wrapping `UserProvider` (needs MUI theme, available to all components)
- **UI**: MUI Snackbar (bottom-center, auto-dismiss 5s) + Alert (`variant="filled"` for dark theme visibility)
- **Severity levels**: `'error' | 'warning' | 'info' | 'success'` (MUI `AlertColor` type)
...
### Key Notes
- `showAlert(message, severity, duration?)` — duration defaults to 5000ms, configurable per-call
...
- Translation keys: `components.alert.newLog.success`, `components.alert.newLog.error`, `components.alert.updateLog.success`, `components.alert.updateLog.error`, `components.alert.deleteLog.success`, `components.alert.deleteLog.error`, `components.alert.watchLogs.error`, `components.alert.generic.error`, `components.alert.generic.networkError`
...
The app is configured as a **Progressive Web App** using `vite-plugin-pwa` with Workbox's `generateSW` strategy.

### Architecture
- **Vite plugin**: `VitePWA` in `vite.config.ts` — generates the service worker at build time (`dist/client/sw.js`)
- **Manifest**: `public/site.webmanifest` — manually managed (not generated by the plugin). Linked via `<link rel="manifest">` in `Layout.tsx`
- **Icons**: `public/img/` — PWA install icons (192x192, 512x512 with both `any` and `maskable` purpose), apple-touch-icon, and favicons
- **SW registration**: `app/components/ServiceWorkerRegistration/ServiceWorkerRegistration.tsx` — dynamically imports `virtual:pwa-register` inside `useEffect`, mounted in `root.tsx`
- **Type declarations**: `app/vite-env.d.ts` — includes `/// <reference types="vite-plugin-pwa/client" />` for `virtual:pwa-register` types

### Plugin Configuration
- `registerType: 'autoUpdate'` — new SW activates automatically on next navigation (no user prompt)
- `injectRegister: false` — SW registration handled manually by the `ServiceWorkerRegistration` component
- `manifest: false` — plugin does not generate/inject a manifest (uses `public/site.webmanifest`)
- `navigateFallback: '/index.html'` — serves the SPA shell for all navigation requests when offline
- `navigateFallbackDenylist: [/^\/api/, /^\/rainlogger-api/]` — prevents SW from intercepting API calls
- `globPatterns: ['**/*.{js,css,html,ico,png,svg,woff,woff2}']` — precaches all static assets
- `devOptions.enabled: true` — SW active during development for testing

### Key Notes
- Theme color `#1a2332` is consistent across `site.webmanifest` (`theme_color`, `background_color`) and `Layout.tsx` (`<meta name="theme-color">`)
- The `ServiceWorkerRegistration` component uses dynamic `import()` with `.catch()` so it safely no-ops in test environments where `virtual:pwa-register` is unavailable
- Build output includes `dist/client/sw.js` and `dist/client/workbox-*.js`
- `workbox-window` is an explicit devDependency (required by `virtual:pwa-register` at build time)
...
- **Provider placement in `root.tsx`**: `ThemeProvider` > `AlertProvider` > `UserProvider` > `ServiceWorkerRegistration` + `<Outlet />`. `ErrorBoundary` is intentionally outside all providers
