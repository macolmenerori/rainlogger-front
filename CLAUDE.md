...
- **Root**: `app/root.tsx` - i18n initialization (side-effect import), font imports, `App` component (with `ThemeProvider` + `AlertProvider` + `UserProvider` + `ServiceWorkerRegistration` + `<Outlet />`), and re-exports of `Layout` and `ErrorBoundary`
...
- **Icons**: `public/icons/` — PWA install icons (192x192, 512x512 with both `any` and `maskable` purpose), apple-touch-icon, and favicons
